...
export function PrayerGrid({
  ...
  const primaryPrayer = currentPrayer ?? nextPrayer;
  const primaryLabel = currentPrayer ? 'Now' : nextPrayer ? 'Next' : 'No prayer';
  const showNextInfo = Boolean(currentPrayer && nextPrayer);
    ...
    <View style={styles.container}>
      ...
      <Card style={[styles.card, { backgroundColor: '#1e4e3c' }]}>
        <View style={styles.cardHeader}>
          <ThemedText style={styles.nowLabel}>{primaryLabel}</ThemedText>
        </View>

        <View style={styles.mainContent}>
          <ThemedText style={styles.prayerName}>{primaryPrayer?.label || 'None'}</ThemedText>
          <View style={styles.weatherRow}>
            ...
            <ThemedText style={styles.prayerTime}>
              {primaryPrayer ? formatTime(primaryPrayer.time) : '--:--'}
            </ThemedText>
          </View>
        </View>

        {showNextInfo && nextPrayer && (
      ...
      <Card
        style={[
          styles.card,
          { backgroundColor: '#1e4e3c', alignItems: 'center', justifyContent: 'center' },
        ]}
      >
