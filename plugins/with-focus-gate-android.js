...
const withFocusGateAndroid = (config) => {
  ...
  config = (0, import_config_plugins.withMainApplication)(config, (mod) => {
...
${importLine}`);
  ...
  config = (0, import_config_plugins.withStringsXml)(config, (mod) => {
    const resources = mod.modResults.resources ?? {};
    const strings = resources.string ?? [];
    const existing = strings.find((entry) => entry?.$?.name === FOCUS_GATE_SERVICE_DESCRIPTION_KEY);
    if (existing) {
...
var with_focus_gate_android_default = withFocusGateAndroid;
